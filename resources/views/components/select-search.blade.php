...
>
    ...
    <button
        class="rounded-md border border-gray-500 content-center p-2 bg-white relative sm:text-sm sm:leading-5 w-full text-left"
    ...
    >
        ...
        @else
            <div class="flex flex-wrap" x-cloak x-show="selected">
                <div class="text-white rounded-full truncate bg-ugreen-500 px-2 py-0.5 my-0.5 flex flex-row items-center">
        ...
        >
            ...
            <div class="bg-white p-2 w-full relative z-30">
                <input x-ref="search" placeholder="Buscar ..." type="search" x-model="search" x-on:click.prevent.stop="open=true" class="block w-full p-2 border border-gray-300 rounded-md focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 sm:text-sm sm:leading-5">
            ...
            <div x-ref="dropdown" class="p-2 max-h-60 overflow-y-auto relative z-30" >
                ...
                <template x-for="(key, index) in Object.keys(options)" :key="index" >
                    @isset($attributes['multiple'])
                        <div
                            class="px-2 py-1"
                            x-bind:class="{'bg-gray-300 text-white hover:none':selected.includes(key), 'hover:bg-ugreen-500 hover:text-white cursor-pointer':!(selected.includes(key)), 'bg-ugreen-500 text-white':currentIndex==index}"
                    ...
                    @else
                        <div
                            class="px-2 py-1"
                            x-bind:class="{'bg-gray-300 text-white hover:none':selected==key, 'hover:bg-ugreen-500 hover:text-white cursor-pointer':!(selected==key), 'bg-ugreen-500 text-white':currentIndex==index}"
                            @click.prevent.stop="selectOption(key)"
                            x-text="Object.values(options)[index]">
                        </div>
                    @endisset
